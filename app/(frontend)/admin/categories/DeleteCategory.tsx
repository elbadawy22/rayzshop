...
import { deleteCategory } from "../../lib/services/client/categories.services";
import Loader from "../../components/loader/Loader";

// import { useRouter } from "next/navigation";

const DeleteCategory = ({ category }: { category: Categories }) => {
  const [open, setOpen] = useState<boolean>(false);
  const [loading, setLoading] = useState<boolean>(false);
  const router = useRouter();
  const handelDeleteCategory = async () => {
    const userDel = await deleteCategory(category.id);
    ...
    <>
      ...
      <>
        <div
          ...
          className={`${!open ? "translate-x-200 sm:translate-x-300 md:translate-x-400  lg:translate-x-700 xl:translate-x-2000 2xl:translate-x-2000 " : ""} px-5 w-screen h-screen flex justify-center  right-[5%] min-w-screen lg:left-[3%] md:left-[3%]  items-center backdrop-blur-xs backdrop-brightness-50  top-0 z-900000 translate-x-5 md:-translate-x-10 transition-all ease-(--my-ease) duration-300  fixed    `}
        >
