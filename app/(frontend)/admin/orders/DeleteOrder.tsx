...
import { deleteOrder } from "../../lib/services/client/orders.services";

// import { useRouter } from "next/navigation";

const DeleteUser = ({ order }: { order: Orders }) => {
  const [open, setOpen] = useState<boolean>(false);
  const router = useRouter();
  const handelDeleteOrder = async () => {
    const userDel = await deleteOrder(order.id);
  ...
  return (
    <>
      <div title="delete order" className=" fixed bottom-5 z-3 w-full  flex justify-end items-center px-5 pb-3   ">
        <div
      ...
      <>
        <div
          ...
          className={`${!open ? "translate-x-200 sm:translate-x-250 md:translate-x-400  lg:translate-x-700 xl:translate-x-2000 2xl:translate-x-2000 " : ""} px-5 w-screen h-screen  flex justify-center  right-[5%] min-w-screen lg:left-[3%] md:left-[3%]  items-center backdrop-blur-xs backdrop-brightness-50  top-0 z-900000 translate-x-5 md:-translate-x-10 transition-all ease-(--my-ease) duration-300  fixed    `}
        >
